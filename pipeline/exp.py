...
class ExpSemantic(ExpBase):
    ...
    def forward(self, batch):
        ...
        if self.training:
            ...
            coords, features_collated = ME.utils.sparse_collate(coordinates, features)
            ...
            tensor_field = ME.TensorField(
                features=features_collated.to(device), 
...
class ExpDGLSS(ExpSemantic):
    def __init__(self, model, cfg):
        ...
        ##@xh
        self.use_contrastive = cfg.train_params.get('use_contrastive', False)
    ...
    def forward(self, batch, use_metric=False):
        ...
        if self.training:
            if 'coordinates_aug' in batch:
                ...
                # 如果没有增强，直接用原始数据
                concat_coordinates = coordinates
    ...
    #     labels_aug = batch.get('labels_aug', None)
    ...
    #     loss = 0
    #     if labels_aug is not None:
    #         labels_cat = torch.cat((labels, labels_aug), dim=0)
    #         current_positive_num = self.positive_num
    #     else:
    #         labels_cat = labels
    #     # segmentation loss
    #     #labels_cat = torch.cat((labels, labels_aug),dim=0)
    ...
    def training_step(self, batch, batch_idx):
        ...
        ##@xh: 修复拼接逻辑，防止 labels_aug 为 None 时报错
        if labels_aug is not None:
            labels_cat = torch.cat((labels, labels_aug), dim=0)
            current_positive_num = self.positive_num
        else:
            labels_cat = labels
            current_positive_num = 0 # 没有增强数据，正样本数量视为 0
        
        # segmentation loss
        # 这里已经删掉了原来那行错误的 labels_cat = torch.cat(...)
        semantic_vox_loss =  self.criterion_sem(out_cat.F, (labels_cat - 1).squeeze().long())
        loss += semantic_vox_loss
        
        # Sparsity Invariant Feature Consistency loss
        consistency_loss = torch.tensor(0.0, device=labels.device)
        if self.use_consistency and labels_aug is not None:
            consistency_loss += losses.SIFCloss(features=enc_bottle_cat, 
                                                batch_size=batch_size, 
                                                positive_num=current_positive_num, 
                                                affinity_thld=self.affinity_threshold)
                    
        consistency_loss = self.consistency_loss_weight * consistency_loss / (max(1, current_positive_num) * batch_size)
        loss += consistency_loss
        ...
        if self.use_correlation:
            ...
            correlation_loss = losses.SCCloss(features=metric_feat, 
                                              ...
                                              batch_ids=batch_idx_list, 
                                              unique_id=torch.unique(batch_idx_list), 
                                              batch_size=batch_size*(1+current_positive_num))
